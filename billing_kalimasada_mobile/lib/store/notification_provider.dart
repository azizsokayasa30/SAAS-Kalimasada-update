...
class NotificationProvider extends ChangeNotifier {
  ...
  Future<void> fetchNotifications({bool silent = false}) async {
    ...
    try {
      final response = await ApiClient.get('/api/mobile-adapter/notifications?limit=80');
  ...
  Future<bool> markRead(dynamic id) async {
    ...
    try {
      final response = await ApiClient.post('/api/mobile-adapter/notifications/$nid/read', {});
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
  ...
  Future<void> markAllRead() async {
    try {
      final response = await ApiClient.post('/api/mobile-adapter/notifications/read-all', {});
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        if (data['success'] == true) {
          for (var i = 0; i < _items.length; i++) {
