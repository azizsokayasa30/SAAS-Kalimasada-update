...
// Bulk mark invoices as paid (Lunas Terpilih)
router.post('/invoices/bulk-pay', adminAuth, async (req, res) => {
    try {
        ...
        const uniqueIds = [...new Set(
            ids.map((rawId) => parseInt(rawId, 10)).filter((id) => Number.isFinite(id) && id > 0)
        )];
        if (uniqueIds.length === 0) {
            return res.status(400).json({ success: false, message: 'Tidak ada ID tagihan yang valid' });
        }
        ...
        for (const id of uniqueIds) {
            try {
                const invoice = await new Promise((resolve, reject) => {
                    billingManager.db.get(
                        `SELECT i.id, i.tenant_id, i.customer_id, i.amount, i.status, i.invoice_number
                         FROM invoices i
                ...
                if (invoice.status === 'cancelled') {
                    throw new Error('Invoice dibatalkan, tidak bisa dilunasi');
                }
                if (invoice.status !== 'unpaid') {
                    throw new Error(`Status ${invoice.status || '-'} tidak bisa dilunasi massal`);
                }
                ...
                try {
                    ...
                    logger.error('Bulk-pay immediate restore check failed:', restoreErr);
                }
        ...
        return res.json({
            success: true,
            summary: { success, failed, skipped, total: uniqueIds.length },
            results
        });
    } catch (error) {
