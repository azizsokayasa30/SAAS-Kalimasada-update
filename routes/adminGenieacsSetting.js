...
// GenieACS Setting page
router.get('/genieacs-setting', adminAuth, async (req, res) => {
  try {
    ...
    // Get GenieACS servers with router count
    ...
    res.render('admin/genieacs-setting', { 
      title: 'GenieACS Setting', 
      servers, 
      page: 'genieacs-setting' 
    });
  } catch (e) {
    res.status(500).render('error', { message: 'Gagal memuat GenieACS Setting', error: e.message });
  }
});

// Add GenieACS server
router.post('/genieacs-setting', adminAuth, async (req, res) => {
  try {
...
router.post('/genieacs-setting/:id', adminAuth, async (req, res) => {
  try {
    const { id } = req.params;
    const { name, url, username, password, description } = req.body;
...
router.post('/genieacs-setting/:id/delete', adminAuth, async (req, res) => {
  try {
    const { id } = req.params;
    const db = require('../config/billing').db;
...
router.post('/genieacs-setting/:id/test', adminAuth, async (req, res) => {
  try {
    const { id } = req.params;
    const db = require('../config/billing').db;
